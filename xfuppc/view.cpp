...
void View::my_readuart()
{
  QByteArray requestData;
  EditWindow->setFontPointSize(10);
  EditWindow->setFontWeight(QFont::DemiBold);
  requestData = my_serialport->readAll();
  if(requestData[14]){
      requestData.remove(13,7);
      StrRequestData = QByteArray(requestData) + " Receive randPoint request\n";
      EditWindow->append(StrRequestData);
      InputDataBtn ->setEnabled(true);//设置标志位当 标志位等于3 时使能input，完成生成随机点
    }
  qDebug()<<"StrRequestData"<<StrRequestData;
